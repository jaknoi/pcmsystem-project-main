        ...
        <div class="card-body">
            ...
            <form action="{{ route('filter.summary') }}" method="GET" class="mb-3">
                <div class="d-flex align-items-center justify-content-center">
                    <label for="year" class="me-2 mb-0">เลือกปี:</label>
                    ...
                    <tbody>
                        ...
                        @if($groupedMonthlyData->isNotEmpty())
                            @foreach($groupedMonthlyData as $month => $data)
                                <tr>
                                    <td>สรุปผลการจัดซื้อจัดจ้างรายเดือน {{ \Carbon\Carbon::parse($month)->locale('th')->translatedFormat('F Y') }}</td>
                                    <td class="text-center">
                                        <a href="{{ route('generate.monthly.pdf', ['month' => \Carbon\Carbon::parse($month)->month, 'year' => \Carbon\Carbon::parse($month)->year]) }}" class="btn btn-pdf">
                            ...
                            @foreach($groupedQuarterlyData as $quarter => $data)
                                <tr>
                                    <td>
                                        ...
                                        @endphp
                                        ประกาศผลผู้ชนะการจัดซื้อจัดจ้าง ประจำไตรมาสที่ {{ $quarterNumber }} ตั้งแต่เดือน {{ \Carbon\Carbon::create()->month($startMonth)->locale('th')->translatedFormat('F') }} ถึงเดือน {{ \Carbon\Carbon::create()->month($endMonth)->locale('th')->translatedFormat('F') }} ปี {{ $year }}
                                    </td>
                                    <td class="text-center">
                                        <a href="{{ route('generate.quarterly.pdf', ['year' => $year, 'quarter' => $quarterNumber]) }}" class="btn btn-pdf">
                                            <i class="fas fa-file-pdf"></i>
                                        </a>
                                    </td>
                                </tr>
...
<style>
    ...
    .modern-card {
        ...
        width: 100%; /* คงขนาดการ์ดให้เต็มตามหน้าจอ */
    }
    ...
    /* General Styles */
    .container {
        max-width: 1000px; /* คงความกว้างของ container */
        margin: 0 auto;
    }
</style>
@endsection
